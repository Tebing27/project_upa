<div class="space-y-8 p-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
            ...
            <p class="mt-2 text-sm text-zinc-500 dark:text-zinc-400">Unggah sertifikat, hasil ujian PDF, dan nilai akhir peserta yang sudah dijadwalkan.</p>
        </div>

        <a href="{{ route('admin.jadwal') }}" wire:navigate class="inline-flex items-center justify-center gap-2 rounded-2xl border border-zinc-300 px-4 py-3 text-sm font-semibold text-zinc-700 shadow-sm transition hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-200 dark:hover:bg-zinc-800">
    ...
    <div class="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h2 class="text-xl font-semibold text-zinc-900 dark:text-white">Peserta Siap Upload</h2>
                <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Pilih peserta terjadwal untuk mengunggah sertifikat dan hasil uji.</p>
            </div>
            ...
            <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800">
                <thead class="bg-zinc-50 dark:bg-zinc-800/60">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500 dark:text-zinc-400">Nilai</th>
                ...
                <tbody class="divide-y divide-zinc-200 bg-white dark:divide-zinc-800 dark:bg-zinc-900">
                    @forelse ($scheduledRegistrations as $registration)
                        <tr wire:key="upload-{{ $registration->id }}" @class([
                            ...
                            <td class="px-4 py-4 text-sm text-zinc-700 dark:text-zinc-300">
                                {{ $registration->score ?? 'Belum diinput' }}
                            </td>
                            <td class="px-4 py-4 text-right">
                                <button wire:click="openUploadModal({{ $registration->id }})" class="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-blue-500">
                                    Upload
                                </button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-10 text-center text-sm text-zinc-500 dark:text-zinc-400">
                                Tidak ada peserta terjadwal yang cocok dengan filter.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                ...
                <div x-show="show"
                    ...
                    <form wire:submit="uploadParticipantFiles" class="flex h-full flex-col">
                        <div class="space-y-6 px-6 py-6">
                            <div>
                                <h3 id="modal-upload-title" class="text-lg font-semibold text-zinc-900 dark:text-white">Upload Sertifikat & Hasil Ujian</h3>
                            ...
                            <div>
                                <label class="mb-1 block text-sm font-medium text-zinc-900 dark:text-white">Nilai Akhir</label>
                                <input wire:model="score" type="number" min="0" max="100" class="block w-full rounded-xl border-0 py-2 text-zinc-900 shadow-sm ring-1 ring-inset ring-zinc-300 focus:ring-2 focus:ring-zinc-900 dark:bg-zinc-800 dark:text-white dark:ring-zinc-700 dark:focus:ring-white sm:text-sm" />
                                @error('score')
                            ...
                            <div class="rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700 dark:border-blue-900/50 dark:bg-blue-950/30 dark:text-blue-200">
                                Setelah disimpan, sertifikat dan hasil ujian langsung tersedia pada dashboard user.
                            </div>
                        </div>
