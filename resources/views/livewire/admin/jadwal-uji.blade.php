<div class="space-y-8 p-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
            ...
            <p class="mt-2 text-sm text-zinc-500 dark:text-zinc-400">Kelola peserta siap dijadwalkan dan peserta yang sudah memiliki jadwal uji.</p>
        </div>

        <a href="{{ route('admin.hasil-uji') }}" wire:navigate class="inline-flex items-center justify-center gap-2 rounded-2xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-500">
            Upload Sertifikat & Hasil
        </a>
    </div>
    ...
    <div class="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Cari peserta berdasarkan nama atau NIM. Filter tanggal hanya diterapkan pada tabel peserta terjadwal.</p>
            </div>
    ...
    <div class="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h2 class="text-xl font-semibold text-zinc-900 dark:text-white">Peserta Siap Dijadwalkan</h2>
                <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Peserta dengan dokumen lengkap bisa langsung dibuatkan jadwal uji.</p>
            </div>
            <span class="inline-flex items-center rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 ring-1 ring-inset ring-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-300 dark:ring-emerald-800/70">
                {{ $eligibleRegistrations->count() }} peserta
            </span>
        </div>
            ...
            <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800">
                <thead class="bg-zinc-50 dark:bg-zinc-800/60">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500 dark:text-zinc-400">Status</th>
                ...
                <tbody class="divide-y divide-zinc-200 bg-white dark:divide-zinc-800 dark:bg-zinc-900">
                    @forelse ($eligibleRegistrations as $registration)
                        <tr wire:key="eligible-{{ $registration->id }}" @class([
                            ...
                            <td class="px-4 py-4">
                                <span class="inline-flex items-center rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700 ring-1 ring-inset ring-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:ring-blue-800/70">
                                    Dokumen OK
                                </span>
                            </td>
                            <td class="px-4 py-4 text-right">
                                <button wire:click="openScheduleModal({{ $registration->id }})" class="inline-flex items-center gap-2 rounded-xl bg-zinc-900 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-zinc-800 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-200">
                                    Jadwalkan
                                </button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="px-4 py-10 text-center text-sm text-zinc-500 dark:text-zinc-400">
                                Tidak ada peserta yang siap dijadwalkan.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div class="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h2 class="text-xl font-semibold text-zinc-900 dark:text-white">Peserta Terjadwal</h2>
                <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Edit atau hapus jadwal per peserta. Upload sertifikat dan hasil ujian dikelola pada halaman terpisah.</p>
            </div>
            <span class="inline-flex items-center rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-700 ring-1 ring-inset ring-amber-200 dark:bg-amber-900/30 dark:text-amber-300 dark:ring-amber-800/70">
                {{ $scheduledRegistrations->count() }} peserta
            </span>
        </div>
            ...
            <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800">
                <thead class="bg-zinc-50 dark:bg-zinc-800/60">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500 dark:text-zinc-400">Skema</th>
                        <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500 dark:text-zinc-400">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-zinc-200 bg-white dark:divide-zinc-800 dark:bg-zinc-900">
                    @forelse ($scheduledRegistrations as $registration)
                        <tr wire:key="scheduled-{{ $registration->id }}" @class([
                            'bg-blue-50/80 dark:bg-blue-900/10' => $highlight === $registration->id,
                            'hover:bg-zinc-50 dark:hover:bg-zinc-800/40' => $highlight !== $registration->id,
                        ])>
                            <td class="px-4 py-4">
                                <p class="font-medium text-zinc-900 dark:text-white">{{ $registration->user->name }}</p>
                            ...
                            <td class="px-4 py-4 text-sm text-zinc-700 dark:text-zinc-300">
                                {{ $registration->exam_date?->translatedFormat('l, d F Y H:i') ?: 'Belum ditentukan' }}
                            </td>
                            <td class="px-4 py-4 text-sm text-zinc-700 dark:text-zinc-300">
                            ...
                            <td class="px-4 py-4 text-sm text-zinc-700 dark:text-zinc-300">
                                {{ $registration->scheme?->name ?: 'Skema belum dipilih' }}
                            </td>
                            <td class="px-4 py-4">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <a href="{{ route('admin.hasil-uji', ['highlight' => $registration->id]) }}" wire:navigate class="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-3 py-2 text-sm font-semibold text-white transition hover:bg-blue-500">
                                        Upload Hasil
                                    </a>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-10 text-center text-sm text-zinc-500 dark:text-zinc-400">
                                Belum ada peserta yang terjadwal.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                ...
                <div x-show="show"
                    ...
                    <form wire:submit="saveSchedule" class="flex h-full flex-col">
                        <div class="space-y-6 px-6 py-6">
                            ...
                            <div class="grid gap-4 md:grid-cols-2">
                                <div>
                                    <label class="mb-1 block text-sm font-medium text-zinc-900 dark:text-white">Tanggal & Waktu Uji</label>
                                    <input wire:model="examDate" type="datetime-local" class="block w-full rounded-xl border-0 py-2 text-zinc-900 shadow-sm ring-1 ring-inset ring-zinc-300 focus:ring-2 focus:ring-zinc-900 dark:bg-zinc-800 dark:text-white dark:ring-zinc-700 dark:focus:ring-white sm:text-sm" />
                                    @error('examDate')
                        ...
                        <div class="flex justify-end gap-3 border-t border-zinc-200 bg-zinc-50 px-6 py-4 dark:border-zinc-800 dark:bg-zinc-800/50">
                            ...
                            <button type="submit" class="rounded-xl bg-zinc-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-zinc-800 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-200">
                                Simpan Jadwal
                            </button>
                        </div>
                    </form>
