...
<head>
    ...
    <style>
        ...
        .fallback-prose .ql-align-center {
            text-align: center;
        }

        .fallback-prose .ql-align-right {
            text-align: right;
        }

        .fallback-prose .ql-align-justify {
            text-align: justify;
        }

        .fallback-prose .ql-font-sans {
            font-family: 'Inter', sans-serif;
        }

        .fallback-prose .ql-font-serif {
            font-family: Georgia, 'Times New Roman', serif;
        }

        .fallback-prose .ql-font-mono {
            font-family: Consolas, 'Courier New', monospace;
        }

        .fallback-prose .ql-font-display {
            font-family: 'Trebuchet MS', Verdana, sans-serif;
            letter-spacing: 0.01em;
        }

        .fallback-prose .ql-size-small {
            font-size: 0.875rem;
        }

        .fallback-prose .ql-size-large {
            font-size: 1.35em;
        }

        .fallback-prose .ql-size-huge {
            font-size: 1.8em;
        }
    </style>
</head>
...
<body class="antialiased bg-slate-50 text-slate-800 flex flex-col min-h-screen">
    ...
    <main class="flex-grow pt-32 pb-24 px-6 container mx-auto">
        <div class="max-w-4xl mx-auto w-full bg-white p-8 md:p-12 rounded-[1.25rem] shadow-sm border border-gray-100">
            ...
            <a href="{{ route('article.index') }}"
                class="inline-flex items-center text-[#ea580c] hover:text-[#c2410c] font-semibold text-sm mb-8 transition-colors">
            ...
            <article class="prose prose-lg prose-slate max-w-none fallback-prose text-gray-700">
                {!! $article->body_segments['before'] !!}
            </article>

            @if (!empty($article->related_articles))
                <div class="my-10 space-y-6">
                    @foreach ($article->related_articles as $relatedArticle)
                        <div class="pl-4 border-l-[3px] border-[#0284c7] py-0.5">
                            <p class="text-[15px] text-slate-600 mb-2 font-medium">Baca Juga:</p>
                            <a href="{{ $relatedArticle->url }}" class="text-[18px] font-bold text-[#0284c7] hover:underline leading-snug block">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            document.querySelectorAll('.fallback-prose img').forEach(img => {
                const captionText = img.getAttribute('data-caption') || img.getAttribute('alt');
                if (captionText && captionText.trim() !== '') {
                    ...
                    // Sisipkan caption langsung di bawah elemen img
