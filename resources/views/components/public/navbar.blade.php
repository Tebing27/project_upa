            ...
            <div class="fixed inset-y-0 right-0 max-w-sm w-full bg-white shadow-2xl flex flex-col"
                ...
                <div class="flex items-center justify-between p-6 border-b border-gray-100 bg-gray-50/50">
                    ...
                    <button @click="mobileMenuOpen = false"
                        class="p-2 text-gray-400 hover:text-gray-800 rounded-md hover:bg-gray-200 transition-colors">
                ...
                <div class="flex-1 overflow-y-auto py-5 px-6 flex flex-col gap-2">
                    @foreach ($navLinks as $key => $link)
                        <a href="{{ route($link['route']) }}"
                            class="py-3 px-4 rounded-xl font-bold transition-colors {{ $active === $key ? 'bg-blue-50 text-[#2563eb]' : 'text-gray-700 hover:bg-gray-50 hover:text-[#2563eb]' }}">
                            {{ $link['label'] }}
                        </a>
                    @endforeach
                </div>
                ...
                <div class="p-6 border-t border-gray-100 flex flex-col gap-4 bg-gray-50">
                    @auth
                        <a href="{{ route('dashboard') }}"
                            class="w-full text-center px-6 py-3.5 rounded-full bg-[#2563eb] text-white hover:bg-[#1d4ed8] font-bold shadow-md transition-all">
                            KE DASHBOARD
                        </a>
                    @else
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}"
                                class="w-full text-center px-6 py-3.5 rounded-full bg-orange-500 text-white hover:bg-orange-600 font-bold shadow-md transition-all">
                                DAFTAR SEKARANG
                            </a>
                        @endif

                        <a href="{{ route('login') }}"
                            class="w-full text-center px-6 py-3.5 rounded-full border-2 border-gray-300 text-gray-700 hover:border-[#2563eb] hover:text-[#2563eb] hover:bg-white font-bold transition-all">
                            MASUK
                        </a>
                    @endauth
                </div>
